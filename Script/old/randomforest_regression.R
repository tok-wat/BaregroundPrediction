...
lm_mod <- lm(BareGround~. , data=df_non_growing_train[,c("BareGround",predictors_all)])
vif(lm_mod)
lm_mod2 <- lm(BareGround~. , data=df_non_growing_train[,c("BareGround","savi","swirRatio","satvi","nir","swir1")])
vif(lm_mod2)
...
temp_df <- df_non_growing_train %>% 
  select(BareGround, all_of(selected))

temp_rfe_tune <- caret::rfe(x = temp_df[,names(temp_df) %in% selected],
...
rfe_1 <- rfe_tune(df_non_growing_train, selected) 
